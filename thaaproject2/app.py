...
captions_df = pd.read_csv(r"A:\AI\aesthetic_instagram_captions.csv")  
...
def index():  
    if request.method == 'POST':  
        if 'foreground' in request.files and 'background' in request.files:  
            ...
            # Remove background and change background  
...
#     img_array = tf.keras.applications.vgg16.preprocess_input(img_array)
#     features = model.predict(img_array)
#     features = features[:, :, :, -1]  # Extract features from last convolutional layer
#     return features
